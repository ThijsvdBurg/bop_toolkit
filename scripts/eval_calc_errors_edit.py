...
p = {
  ...
  'targets_filenames_MPPI': [
    ...
    'test_targets_bop19_WIP_010029.json',
...
#   split_type = 'test'
  ...
  for scene_id, scene_targets in targets_org.items():
    ...
    print('loading scene gt from',dp_split['scene_gt_tpath'].format(scene_id=scene_id))
    ...
    for im_ind, (im_id, im_targets) in enumerate(scene_targets.items()):
      ...
      for obj_id, target in im_targets.items():
        ...
        for est_id, est in obj_ests_sorted:
          ...
          ### PMB debug ###
          # print('t_estimate for scene id and im id\n',t_e,'\n',scene_id,'\n',im_id)
          ...
          for gt_id, gt in enumerate(scene_gt[im_id]):
            ...
            bbox_g = scene_gt_info[im_id][0]['bbox_obj']

            azimuth = est['azimuth']
            alitude = est['altitude']

            pnpinit = est['pnp_init']
            pnpiter = est['pnp_iters']
          ...
          scene_errs.append({
            ...
            'est_id': est_id,
            'score': est['score'],
            'pnp_init': est['pnp_init'],
            'pnp_iters': est['pnp_iters'],
            'azimuth': azimuth,
            'altitude': alitude,
            'errors': errs,
            'IoU': IoUs
    ...
    # def save_bop(_error_sign, _scene_errs):
    #   Save the calculated errors to a JSON file.
    #   errors_path = p['datasets_path']
    #   #.format(
    #   # eval_path=p['eval_path'], result_name=result_name,
    #   # error_sign=_error_sign, scene_id=scene_id)
    #   print('errors path', errors_path)
